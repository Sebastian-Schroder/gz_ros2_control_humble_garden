...
public:
  ...
  std::map<std::string, SIM_NAMESPACE::Entity> GetEnabledJoints(
    const SIM_NAMESPACE::Entity & _entity,
    SIM_NAMESPACE::EntityComponentManager & _ecm) const;

  /// \brief Entity ID for sensor within Gazebo.
  SIM_NAMESPACE::Entity entity_;
  ...
  /// \brief ECM pointer
  SIM_NAMESPACE::EntityComponentManager * ecm{nullptr};
...
//////////////////////////////////////////////////
std::map<std::string, SIM_NAMESPACE::Entity>
IgnitionROS2ControlPluginPrivate::GetEnabledJoints(
  const SIM_NAMESPACE::Entity & _entity,
  SIM_NAMESPACE::EntityComponentManager & _ecm) const
{
  std::map<std::string, SIM_NAMESPACE::Entity> output;

  std::vector<std::string> enabledJoints;

  // Get all available joints
  auto jointEntities = _ecm.ChildrenByComponents(_entity, SIM_NAMESPACE::components::Joint());

  // Iterate over all joints and verify whether they can be enabled or not
  for (const auto & jointEntity : jointEntities) {
    const auto jointName = _ecm.Component<SIM_NAMESPACE::components::Name>(
      jointEntity)->Data();

    // Make sure the joint type is supported, i.e. it has exactly one
    // actuated axis
    const auto * jointType = _ecm.Component<SIM_NAMESPACE::components::JointType>(jointEntity);
...
void IgnitionROS2ControlPlugin::PostUpdate(
  const SIM_NAMESPACE::UpdateInfo & _info,
  const SIM_NAMESPACE::EntityComponentManager & /*_ecm*/)
{
